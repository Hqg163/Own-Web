<template>
  <main class="container reading page-section">
    ...
    <div v-else-if="post" class="article-layout">
      ...
      <aside v-if="toc.length" ref="tocWrap" class="toc-wrap">
        <details class="toc-mobile card"><summary>本文目录 <span>{{ toc.length }}</span></summary><nav aria-label="文章目录"><a v-for="item in toc" :key="item.id" :href="`#${item.id}`" :class="{ active: activeHeading === item.id, [`toc-level-${item.level}`]: true }" @click.prevent="jumpTo(item.id)">{{ item.text }}</a></nav></details>
        <nav class="toc card" aria-label="文章目录"><strong>目录</strong><a v-for="item in toc" :key="item.id" :href="`#${item.id}`" :class="{ active: activeHeading === item.id, [`toc-level-${item.level}`]: true }" @click.prevent="jumpTo(item.id)">{{ item.text }}</a></nav>
      </aside>
    </div>
...
import { sameCommentId } from '@/components/comments/format'
...
const commentMedia = ref<Array<{ id?: number; url: string; name?: string; alt?: string; file?: File }>>([])
...
const reportTargetId = ref<number | string>(0)
const reportTargetLabel = ref('')
const progress = ref(0)
const toc = ref<TocHeading[]>([])
const activeHeading = ref('')
const tocWrap = ref<HTMLElement>()
...
function normalizeCommentItems(items: unknown): Comment[] { return Array.isArray(items) ? items as Comment[] : [] }
async function loadComments(append = false) { if (!post.value) return; if (append) commentsLoadingMore.value = true; else { commentsLoading.value = true; replyCursors.value = {} }; commentError.value = ''; const apiSort = commentSort.value === 'newest' ? 'latest' : commentSort.value === 'popular' ? 'hot' : 'oldest'; const requestedPage = append ? commentsPage.value + 1 : 1; try { const response = await http.get(`/api/posts/${post.value.id}/comments`, { ...shareConfig(), params: { ...(shareConfig().params || {}), sort: apiSort, page: requestedPage, pageSize: 20 } }); const nextItems = normalizeCommentItems(response.data?.items); comments.value = append ? [...comments.value, ...nextItems] : nextItems; commentsPage.value = Number(response.data?.page || requestedPage); commentCount.value = Number(response.data?.total ?? response.data?.commentCount ?? post.value.comment_count ?? comments.value.length); const size = Number(response.data?.pageSize || 20); const rootTotal = Number(response.data?.rootTotal ?? comments.value.filter((item) => !item.parent_id).length); hasMoreComments.value = commentsPage.value * size < rootTotal } catch (e: any) { commentError.value = e.response?.data?.error?.message || '评论暂时无法载入。' } finally { commentsLoading.value = false; commentsLoadingMore.value = false } }
...
function reportComment(comment: any) { if (!loggedIn.value) return login(); reportTargetType.value = 'comment'; reportTargetId.value = Number(comment.id); reportTargetLabel.value = String(comment.content || '').trim().slice(0, 120) || '这条评论'; reportDialogOpen.value = true }
function closeReportDialog() { reportDialogOpen.value = false; actionPending.value.report = false }
function handleReportSubmitted() { if (reportTargetType.value === 'post') reported.value = true; closeReportDialog(); setActionMessage('举报已提交，我们会按规则处理。') }
async function removeComment(comment: any) { deleteCommentId.value = Number(comment.id); await nextTick(); deleteConfirmButton.value?.focus() }
...
function report() { if (!loggedIn.value) return login(); if (!post.value || actionPending.value.report || reported.value) return; reportTargetType.value = 'post'; reportTargetId.value = post.value.id; reportTargetLabel.value = post.value.title; reportDialogOpen.value = true }
