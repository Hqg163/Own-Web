...
const Details=Node.create({name:'details',group:'block',atom:true,addAttributes(){return{summary:{default:'展开阅读'},body:{default:''}}},parseHTML(){return[{tag:'details'}]},renderHTML({HTMLAttributes}){return['details',{'data-details':'true'},['summary',HTMLAttributes.summary],['p',HTMLAttributes.body]]}})
const SafeEmbed=Node.create({name:'embed',group:'block',atom:true,addAttributes(){return{provider:{default:''},url:{default:''}}},parseHTML(){return[{tag:'a[data-embed]'}]},renderHTML({HTMLAttributes}){return['a',mergeAttributes({'data-embed':'true',href:HTMLAttributes.url,target:'_blank',rel:'noopener'},HTMLAttributes),HTMLAttributes.url]}})
...
const tools=computed(()=>[
  {label:'标题',icon:'pen',action:()=>editor.value?.chain().focus().toggleHeading({level:2}).run()},{label:'粗体',icon:'pen',action:()=>editor.value?.chain().focus().toggleBold().run()},{label:'斜体',icon:'pen',action:()=>editor.value?.chain().focus().toggleItalic().run()},{label:'删除线',icon:'close',action:()=>editor.value?.chain().focus().toggleStrike().run()},{label:'链接',icon:'external-link',action:insertLink},{label:'引用',icon:'more',action:()=>editor.value?.chain().focus().toggleBlockquote().run()},{label:'项目列表',icon:'list',action:()=>editor.value?.chain().focus().toggleBulletList().run()},{label:'编号列表',icon:'list',action:()=>editor.value?.chain().focus().toggleOrderedList().run()},{label:'提示卡',icon:'info',action:insertCallout},{label:'折叠内容',icon:'chevron-down',action:insertDetails},{label:'受控嵌入',icon:'external-link',action:insertEmbed},{label:'代码块',icon:'code',action:()=>editor.value?.chain().focus().toggleCodeBlock().run()},{label:'分割线',icon:'more',action:()=>editor.value?.chain().focus().setHorizontalRule().run()},{label:'表格',icon:'grid',action:()=>editor.value?.chain().focus().insertTable({rows:3,cols:3,withHeaderRow:true}).run()},{label:'撤销',icon:'rotate-ccw',action:()=>editor.value?.chain().focus().undo().run()},{label:'重做',icon:'rotate-cw',action:()=>editor.value?.chain().focus().redo().run()}
])
...
function makeEditor(content:any=null){editor.value?.destroy();editor.value=new Editor({extensions:[StarterKit.configure({link:false}),Image.configure({allowBase64:false}),Link.configure({openOnClick:false}),Table.configure({resizable:true}),TableRow,TableHeader,TableCell,Callout,Details,SafeEmbed,Markdown],content:content||{type:'doc',content:[{type:'paragraph'}]},onUpdate:()=>{dirty.value=true;queueAutosave()}})}
function blocks(){return editor.value?.getJSON()||{type:'doc',content:[{type:'paragraph'}]}}
function insertLink(){const href=window.prompt('输入以 http:// 或 https:// 开头的链接');if(!href)return;if(!/^https?:\/\//i.test(href.trim())){error.value='链接仅支持 HTTP 或 HTTPS 地址';return}editor.value?.chain().focus().extendMarkRange('link').setLink({href:href.trim()}).run()}
function insertCallout(){editor.value?.chain().focus().insertContent({type:'callout',attrs:{tone:'info'},content:[{type:'text',text:'提示内容'}]}).run()}
function insertDetails(){const summary=window.prompt('输入折叠标题','展开阅读');if(!summary?.trim())return;const body=window.prompt('输入折叠内容','')||'';editor.value?.chain().focus().insertContent({type:'details',attrs:{summary:summary.trim(),body}}).run()}
function insertEmbed(){const url=window.prompt('输入 Bilibili、YouTube 或 Spotify 的 HTTPS 链接');if(!url)return;try{const parsed=new URL(url.trim());if(!['www.youtube.com','youtu.be','www.bilibili.com','open.spotify.com'].includes(parsed.hostname)||parsed.protocol!=='https:')throw new Error();editor.value?.chain().focus().insertContent({type:'embed',attrs:{provider:parsed.hostname,url:parsed.toString()}}).run()}catch{error.value='仅支持 Bilibili、YouTube 或 Spotify 的 HTTPS 链接'}}
...
.rich-editor :deep(.editor-callout){margin:var(--space-3) 0;padding:var(--space-3);border:1px solid var(--accent);border-left:3px solid var(--accent);border-radius:var(--radius-sm);background:var(--accent-soft);color:var(--text)}.rich-editor :deep(details){margin:var(--space-3) 0;padding:var(--space-3);border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--surface-raised)}.rich-editor :deep(details summary){cursor:pointer;color:var(--accent);font-weight:700}.rich-editor :deep(a[data-embed]){display:block;padding:var(--space-3);border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--surface-raised);color:var(--accent);overflow-wrap:anywhere}
</style>
