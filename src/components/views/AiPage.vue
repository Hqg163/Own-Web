    ...
    <div v-else class="ai-page__workspace">
      ...
      <section class="ai-page__chat" aria-label="当前 AI 对话">
        <header class="ai-page__chat-head">
          ...
          <label for="ai-page-model">
            <span>模型</span>
            <select id="ai-page-model" v-model="ai.state.selectedModel" :disabled="ai.isStreaming.value" @change="persistSelectedModel">
...
async function persistSelectedModel() {
  if (!ai.state.conversationId) return
  try { await (ai as any).updateConversation?.(ai.state.conversationId, { selectedModel: ai.state.selectedModel }) } catch (_) {}
}
</script>

<style scoped>
.ai-page.container { width: min(calc(100% - 32px), 1540px); max-width: none; padding-bottom: 0; }.ai-page__header { display: flex; align-items: end; justify-content: space-between; gap: var(--space-4); }.ai-page__header h1 { margin-bottom: var(--space-2); }.ai-page__header p:last-child { max-width: 650px; margin: 0; color: var(--muted); }.ai-page__header-actions { display: flex; flex-wrap: wrap; gap: var(--space-2); }.ai-history-toggle { display: none; }
...
@media (max-width: 760px) { .ai-page.container { width: 100%; padding-inline: var(--space-3); }.ai-page__header { align-items: start; flex-direction: column; }.ai-history-toggle { display: inline-flex; }.ai-page__workspace { display: block; height: calc(100dvh - 185px); min-height: 500px; margin-inline: calc(var(--space-3) * -1); }.ai-page__history { position: fixed; z-index: 35; top: 0; bottom: 0; left: 0; display: none; width: min(320px, 88vw); padding: var(--space-5) var(--space-3); border: 0; border-right: 1px solid var(--border); background: var(--surface); box-shadow: var(--shadow); }.ai-page__history.is-open { display: block; }.ai-page__chat { height: 100%; min-height: 0; }.ai-page__chat-head { flex-direction: column; padding-inline: var(--space-3); }.ai-page__chat-head label, .ai-page__chat-head select { width: 100%; max-width: none; } }
</style>
