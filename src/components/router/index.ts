...
router.beforeEach(async(to)=>{const cached=localStorage.getItem('isLoggedIn')==='true';let logged=false;if(cached||to.meta.requiresAuth||to.meta.guestOnly){try{const {data}=await http.get('/api/me');cacheAuthenticatedUser(data.user, data.capabilities);logged=true}catch(error:any){if(error.response?.status===401)clearCachedAuth();else logged=cached}}if(to.meta.requiresAuth&&!logged){localStorage.setItem('redirectAfterLogin',to.fullPath);return'/login'}if(to.meta.guestOnly&&logged)return'/dashboard';return true})
