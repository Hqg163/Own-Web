<template>
  <section class="ai-chat-surface" :class="{ compact }" aria-label="AI 对话">
    ...
    <form class="ai-chat-surface__composer" @submit.prevent="submit">
      ...
      <textarea :id="id" ref="composer" v-model="draft" rows="3" maxlength="8000" placeholder="问问文章内容，或直接开始聊天…" :disabled="ai.isStreaming.value || !canChat" @keydown.enter.exact.prevent="submit"></textarea>
...
.ai-chat-surface__messages { display: grid; align-content: start; gap: var(--space-3); min-height: 220px; overflow: auto; padding: var(--space-1) var(--space-1) var(--space-2); }
...
.ai-chat-surface__composer { display: grid; gap: var(--space-2); padding-top: var(--space-2); border-top: 1px solid var(--border); }.ai-chat-surface__composer textarea { width: 100%; min-height: 76px; padding: var(--space-2) var(--space-3); resize: vertical; border: 1px solid var(--border); border-radius: var(--radius-sm); background: var(--surface-raised); color: var(--text); }.ai-chat-surface__composer textarea:focus { border-color: var(--accent); outline: 3px solid color-mix(in srgb, var(--accent), transparent 72%); outline-offset: 1px; }.ai-chat-surface__compose-footer { display: flex; align-items: center; justify-content: space-between; gap: var(--space-2); color: var(--muted); font-size: .75rem; }.ai-chat-surface__compose-footer > div { display: flex; gap: var(--space-2); }.ai-chat-surface__compose-footer .button { min-height: 34px; padding: 0 10px; font-size: .84rem; }
@media (max-width: 640px) { .ai-chat-surface__messages { min-height: 180px; }.ai-message { max-width: 96%; } }
</style>
