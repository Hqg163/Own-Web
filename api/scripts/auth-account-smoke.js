...
const db = mysql.createConnection({ host: process.env.DB_HOST, port: Number(process.env.DB_PORT || 3306), user: process.env.DB_USER, password: process.env.DB_PASSWORD, database: process.env.DB_NAME });
...
async function request(pathname, options = {}) { const response = await fetch(`http://127.0.0.1:${port}${pathname}`, options); const text = await response.text(); let body; try { body = text ? JSON.parse(text) : null; } catch (_) { body = text; } return { status: response.status, body, headers: response.headers }; }
...
async function run() {
  server = spawn(process.execPath, ['server.js'], { cwd: apiRoot, env: { ...process.env, PORT: String(port) }, stdio: ['ignore', 'ignore', 'ignore'], windowsHide: true });
  await waitForServer();
  ...
  await expectStatus('/api/register', 400, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ email: email.toLowerCase(), password }) });
