...
function createRetriever({ db, config, qdrant, embeddingProvider, rerankerProvider, retrievalCache }) {
  ...
  async function hydrate(points, user, context) {
    ...
    const [rows] = await query(
      `SELECT c.*,p.author_id,p.status,p.visibility,p.share_token,p.title,p.slug,p.published_at,p.updated_at
    ...
    try {
      response = await qdrant.client.query(qdrant.collection, {
        prefetch: [
          ...
          { query: { text: question, model: 'Qdrant/bm25', options: { tokenizer: 'multilingual' } }, using: 'bm25', limit: 20, filter: scope.filter },
