...
function validateUploadedFile(file, { allowed, maxBytes, rejectSvg = true } = {}) {
  ...
  if (detected.startsWith('image/')) {
    ...
    if (detected === 'image/png' && bytes.lastIndexOf(Buffer.from('IEND')) !== bytes.length - 8) throw Object.assign(new Error('PNG 文件不完整或包含尾随数据'), { status: 400, code: 'FILE_CONTENT_INVALID' });
    if (detected === 'image/gif' && bytes[bytes.length - 1] !== 0x3b) throw Object.assign(new Error('GIF 文件不完整或包含尾随数据'), { status: 400, code: 'FILE_CONTENT_INVALID' });
    if (detected === 'image/jpeg' && !(bytes.length >= 2 && bytes[bytes.length - 2] === 0xff && bytes[bytes.length - 1] === 0xd9)) throw Object.assign(new Error('JPEG 文件不完整或包含尾随数据'), { status: 400, code: 'FILE_CONTENT_INVALID' });
  }
