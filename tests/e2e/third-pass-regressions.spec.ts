...
test('article TOC scrolls below the sticky header and renders math', async ({ page }) => {
  ...
  expect(ids[0]).toBe('介绍')
  expect(ids[1]).toBe('介绍-2')
  const desktopTocLink = page.locator('.toc a.toc-level-2').first()
  if (await desktopTocLink.isVisible()) {
    ...
    await mobileToc.locator('a.toc-level-2').first().click()
  }
  ...
  const top = await headings.first().evaluate((node) => node.getBoundingClientRect().top)
