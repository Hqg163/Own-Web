...
async function main() {
  ...
  try {
    ...
    assert.equal('details' in ownReport, false)
    assert.equal('target_snapshot' in ownReport, false)
    assert.equal('media' in ownReport, false)
    assert.match(String(ownReport.summary), /举报 API 文章/)
    ...
    assert.match(JSON.stringify(ownDetail.body.report.target_snapshot), /举报 API 文章/)
    assert.doesNotMatch(JSON.stringify(ownDetail.body.report.target_snapshot), /已修改/)
    assert.doesNotMatch(JSON.stringify(ownDetail.body.report.target_snapshot), /private source/)
