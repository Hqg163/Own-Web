...
async function main() {
  ...
  try {
    ...
    const forbiddenDelete = await reader.delete(`/api/posts/${id}`).set('Origin', origin)
    assert.equal(forbiddenDelete.status, 404)
    const draftForDelete = await owner.post('/api/posts').set('Origin', origin).send({ title:'待删除草稿', contentFormat:'markdown', contentMarkdown:'删除回归正文' })
    assert.equal(draftForDelete.status, 201)
    const deletedDraft = await owner.delete(`/api/posts/${draftForDelete.body.post.id}`).set('Origin', origin)
    assert.equal(deletedDraft.status, 204)
    const missingDraft = await owner.get(`/api/posts/${draftForDelete.body.post.id}`).set('Origin', origin)
    ...
    const publishedDelete = await owner.delete(`/api/posts/${id}`).set('Origin', origin)
    assert.equal(publishedDelete.status, 409); assert.equal(publishedDelete.body.error.code, 'POST_NOT_DELETABLE')
