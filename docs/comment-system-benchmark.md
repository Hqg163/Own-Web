# 评论系统交互基准（第三轮设计依据，第四轮补充）

> 记录日期：2026-08-30（第三轮基准，第四轮补充）。本文是交互取舍记录，不是对任何平台全部功能的完整复刻，也不构成 Own-Web 生产行为的验证证据。引用页面由任务需求提供，结论只提炼通用模式。

本文件只提炼公开平台的通用交互模式，不复制品牌、颜色、Logo 或页面布局。Own-Web 采用自己的编辑型视觉系统。

## 第四轮实现补充

第四轮将这些通用模式落到可审核的 Own-Web 流程：举报类型使用稳定 `reason_code` 而不是展示文案；举报保存短目标快照和独立证据媒体；用户只能查看自己的举报与公开处理说明，管理员内部备注单独隔离；`pending/reviewing` 重复举报会被拒绝并受用户/IP 限流；终态会产生 `report_update` 通知。文章和评论共用一个 Report Dialog，证据 URL 仍按举报人/管理员权限授权，不成为 private 内容的泄露入口。以上是当前代码和本地测试范围，不是生产投递或跨账号验证声明。
